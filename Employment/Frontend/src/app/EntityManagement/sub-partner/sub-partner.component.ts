...
export class SubPartnerComponent implements OnInit {
  ...
  ReconstructeTrainingCenterList() {
    ...
    this.modelObjSubPartner.Partners.forEach(value => {
      value.TrainingCenters.length ? this.TrainingCenterList = this.TrainingCenterList.concat(value.TrainingCenters) : noop;
    });
  ...
  AddAllPartnerTrainingCenter(PartnerId: string) {
    if (!confirm("Are you sure you want to add all?"))
      return;
  ...
  filterSubPartnerAccount(obj:cUserProfileItem[]){;
    ...
    var x = obj.filter(x=>x.Type == UserType.SubPartner);;
    return obj.filter(x=>x.Type == UserType.SubPartner);
  }
}
