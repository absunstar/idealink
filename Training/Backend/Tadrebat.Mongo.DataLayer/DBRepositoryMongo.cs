...
{
    public class DBRepositoryMongo<T> : DBBase, IRepositoryMongo<T> where T : MongoEntityBase
    {
        ...
        {
            var obj = await _mongoCollection.FindSync(filter).FirstOrDefaultAsync();
            return obj;
        ...
        public async Task<T> GeOne(FilterDefinition<T> filter)
        {
            var obj = await _mongoCollection.FindSync(filter).FirstOrDefaultAsync();
            return obj;    
        }
        ...
        public async Task<long> GetCount(FilterDefinition<T> filter)
        {
            var result = await _mongoCollection.CountAsync(filter);
            return result;
        }
